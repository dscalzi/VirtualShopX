...
public class ConversionUtil {
	...
	public static void merge() throws FileNotFoundException, IOException {
		...
		try(JsonReader jr = new JsonReader(new FileReader(target));
			FileReader reader = new FileReader(file);
			BufferedReader rx = new BufferedReader(reader)){
			
			JsonElement parsed = p.parse(jr);
			JsonArray arr = parsed.getAsJsonObject().get("items").getAsJsonArray();
			
			JsonObject container = new JsonObject();
			
			final String currVer = Bukkit.getBukkitVersion().split("-")[0];
				...
				try {
					...
					for(JsonElement e : arr) {
						if(e.isJsonObject()) {
							...
							if(eO.has("materialSpigot") && eO.get("materialSpigot").getAsString().equals(iStack.getType().name()) &&
							   eO.has("legacyData") && eO.get("legacyData").getAsString().equals(parts[2]) &&
							   (!isPotionable(iStack) || parts.length < 4)){
								container = eO;
								JsonArray vers = container.get("versions").getAsJsonArray();
								vers.add(currVer);
								container.add("versions", vers);
								continue grand;
							}
		...
		try(FileReader reader = new FileReader(file);
			...
			for (int i = 0; rx.ready(); i++){
				try {
					...
					if(container.has("materialSpigot") && container.get("materialSpigot").getAsString().equals(iStack.getType().name()) &&
					...
					first = false;
					container.addProperty("materialSpigot", iStack.getType().name());
